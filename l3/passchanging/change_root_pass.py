...
class ChangeRootPassword():
    ...
    def __init__(self,hostList,credentials,newRootPassword,pwdComplexityCheck=True,logfile=None):
    ...
    def changePasswordSSH(self,relay=None,relayCredentials=[]):
        ...
        for user,password in self.credentials:
            ...
            passwdCommand = 'passwd {user}'.format(user=self.ROOT)
...
def qproducer(idnum,changeRootPasswordObject):
    ...
    for host in changeRootPasswordObject.hostList:
        currentTask = cpTask(host,changeRootPasswordObject.newRootPassword,changeRootPasswordObject.credentials,relayHost,relayCredentials,changeRootPasswordObject.logger)
...
class cpTask():
    ...
    def __init__(self,host,newPassword,credentials,defRelayHost=None,defRelayCredentials=(),logger=None):
    ...
    def start(self):
        ...
        if relayHost or port_ssh_host_avail:
            ...
            res = ChangeRootPassword([self.host],self.credentials,self.newPassword,pwdComplexityCheck=False,logfile=self.logger.get_logfile()).changePasswordSSH()
            if res:
...
if __name__=="__main__":
    ...
    new_root_password = getpass.getpass("New Root password: ")
    if  new_root_password:
        r_new_root_password = getpass.getpass("Repeat new root password: ")
        if not new_root_password == r_new_root_password:
    ...
    if i == start_phrase:
        ...
        logger(log_file).write(run_host,0,"Root password change started from {0} at {1} by {2}".format(socket.gethostname(),time_str,run_user))
        m = ChangeRootPassword(host_list,credentials,new_root_password,logfile=log_file)
        m.start()
